...
def get_cve(scanmd5):
	cvehost = {}
	cvefiles = os.listdir('/root/TOOLZ/THSMap/notes')
	for cf in cvefiles:
		...
		if m is not None:
			...
			if m.group(2) not in cvehost[m.group(1)]:
				cvehost[m.group(1)][m.group(2)] = open('/root/TOOLZ/THSMap/notes/'+cf, 'r').read()

			#cvehost[m.group(1)][m.group(2)][m.group(3)] = open('/root/TOOLZ/THSMap/notes/'+cf, 'r').read()
