...
def reportPDFView(request):
    ...
    for ik in o['host']:
        ...
        if i['status']['@state'] == 'up':
            ...
            hostdetails_html += '	<h2 id="addr' + addressmd5 + '">' + html.escape(
                saddress) + ' ' + labelout + '</h2> '
        ...
        if 'ports' in i and 'port' in i['ports']:
            for pobj in i['ports']['port']:
                ...
                hostdetails_html_tr += '<tr>' + \
                                       '	<td><span class="' + hdhtml_protocolor + '-text">' + p[
                                           '@protocol'] + '</span> / <span class=""><b>' + p[
                                           '@portid'] + '</b></span><br><span class="small">' + servicename + '</span></td>' + \
                ...
                if 'script' in p:
                    ...
                    for ii in p['script']:
                        ...
                        if script['@output'].replace('\n', '') != '' and script['@id'] not in filterscriptid:
                            portdetails_html_tr += '<div class="extratitle"><b class="red-text">' + html.escape(
                                script['@id']) + '</b> - Address: <b>' + html.escape(saddress) + '</b> - Port: <b>' + p[
                                                       '@portid'] + '</b></div>' + \
        ...
        if i['status']['@state'] == 'up':
            hostdetails_html += '<div class="row margintb">' + \
                                '		<div class="col s3"><center><h3><i class="fab fa-creative-commons-sampling"></i> ' + str(
                hostcounters['po'] + hostcounters['ports_closed '] + hostcounters[
                    'pf']) + '</h3>TOTAL PORT</center></div>' + \
                                '		<div class="col s3 bleft""><center><h3><i class="fas fa-door-open green-text"></i> ' + str(
                hostcounters['po']) + '</h3>OPEN PORT</center></div>' + \
                                '		<div class="col s3 bleft"><center><h3><i class="fas fa-door-closed red-text"></i> ' + str(
                hostcounters['ports_closed ']) + '</h3>CLOSED PORT</center></div>' + \
                                '		<div class="col s3 bleft"><center><h3><i class="fas fa-filter grey-text"></i> ' + str(
                hostcounters['pf']) + '</h3>FILTERED PORT</center></div>' + \
                                '	</div>' + \
    ...
    r[
        'html'] += '<script type="text/javascript" src="https://www.google.com/jsapi?autoload={%27modules%27:[{%27name%27:%27visualization%27,%27version%27:%271.1%27,%27packages%27:[%27corechart%27,%27sankey%27,%27annotationchart%27]}]}"></script>' + \
                   ...
                   '		<div class="col s3"><b class="blue-text" style="font-size:24px;">HOSTS UP</b><br><span style="color:#999;font-size:32px;">' + str(
        counters['hostsup']) + '</span></div>' + \
                   '		<div class="col s3" style="border-left:solid #ccc 1px;"><b class="blue-text" style="font-size:24px;">PORTS</b><br><span style="color:#999;font-size:32px;">' + str(
        counters['ports_closed '] + counters['po'] + counters['pf']) + '</span></div>' + \
                   '		<div class="col s3" style="border-left:solid #ccc 1px;"><b class="blue-text" style="font-size:24px;">SERVICES</b><br><span style="color:#999;font-size:32px;">' + str(
        len(counters['ss'].keys())) + '</span></div>' + \
                   '		<div class="col s3" style="border-left:solid #ccc 1px;"><b class="blue-text" style="font-size:24px;">OS</b><br><span style="color:#999;font-size:32px;">' + str(
        len(counters['ostype'].keys())) + '</span></div>' + \
                   '	</div>' + \
                   ...
                   '			<b class="subtitle">Total Ports</b><br><span class="subtitle">' + str(
        counters['ports_closed '] + counters['po'] + counters['pf']) + '</span><br><br>' + \
                   '			<b class="subtitle green-text">Open Ports</b><br><span class="subtitle green-text">' + str(
        counters['po']) + '</span><br><br>' + \
                   '			<b class="subtitle red-text">Closed Ports</b><br><span class="subtitle red-text">' + str(
        counters['ports_closed ']) + '</span><br><br>' + \
                   '			<b class="subtitle orange-text">Filtered Ports</b><br><span class="subtitle orange-text">' + str(
        counters['pf']) + '</span><br><br>' + \
                   '			<b>Ports</b>:<br><span style="font-family:monospace;font-size:11px;">' + html_ports[
                                                                                                         0:-2] + '</span><br><br>' + \
                   '			<b>Services</b>:<br><span style="font-family:monospace;font-size:11px;">' + html_services[
                                                                                                            0:-2] + '</span>' + \
                   '		</div>' + \
                   '</div>'
    ...
    return render(request, 'thsdashboard/report.html', r)
